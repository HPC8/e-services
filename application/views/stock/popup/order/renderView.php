                            ...
                            <span>
                                <?php 
                                    ...
                                    if ( !empty($data->approve_id)) {  
                                        echo '<br>'.'<span class="badge badge-success">2</span> '.$thaidate->thai_date_and_time($data->approve_date);
                                        echo 'โดย'.get_instance()->user_model->getUsername($data->approve_id);
                                    }
                                    if ( !empty($data->supplies_id)) {  
                                        echo '<br>'.'<span class="badge badge-warning">3</span> '.$thaidate->thai_date_and_time($data->supplies_date);
                                        echo 'โดย'.get_instance()->user_model->getUsername($data->supplies_id);
                                    }
                                    if ( !empty($data->send_id)) {  
                                        echo '<br>'.'<span class="badge badge-yellow">4</span> '.$thaidate->thai_date_and_time($data->send_date);
                                        echo 'โดย'.get_instance()->user_model->getUsername($data->send_id);
                                    }
                                    if ( !empty($data->receive_id)) {  
                                        echo '<br>'.'<span class="badge badge-info">5</span> '.$thaidate->thai_date_and_time($data->receive_date);
