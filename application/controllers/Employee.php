<?php Class Employee extends CI_Controller {
    ...
    public function Export() {
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('J1', 'เลขบัตรประชาชน');
        $objPHPExcel->getActiveSheet()->SetCellValue('K1', 'อีเมล์');
        $objPHPExcel->getActiveSheet()->SetCellValue('L1', 'เบอร์โทรศัพท์');
        $objPHPExcel->getActiveSheet()->SetCellValue('M1', 'ที่อยู่');
        $objPHPExcel->getActiveSheet()->SetCellValue('N1', 'ตำบล');
        $objPHPExcel->getActiveSheet()->SetCellValue('O1', 'อำเภอ');
        $objPHPExcel->getActiveSheet()->SetCellValue('P1', 'จังหวัด');
        $objPHPExcel->getActiveSheet()->SetCellValue('Q1', 'รหัสไปรษณีย์');
        $objPHPExcel->getActiveSheet()->SetCellValue('R1', 'ประเภทบุคลากร');
        $objPHPExcel->getActiveSheet()->SetCellValue('S1', 'ตำแหน่ง');
        $objPHPExcel->getActiveSheet()->SetCellValue('T1', 'ระดับ');
        $objPHPExcel->getActiveSheet()->SetCellValue('U1', 'กลุ่ม');
        $objPHPExcel->getActiveSheet()->SetCellValue('V1', 'งาน');
        ...
        foreach ($empInfo as $element) {
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('J'. $rowCount, $element->cid);
            $objPHPExcel->getActiveSheet()->SetCellValue('K'. $rowCount, $element->email);
            $objPHPExcel->getActiveSheet()->SetCellValue('L'. $rowCount, $element->mobile);
            $objPHPExcel->getActiveSheet()->SetCellValue('M'. $rowCount, $element->address);
            $objPHPExcel->getActiveSheet()->SetCellValue('N'. $rowCount, $element->district_name);
            $objPHPExcel->getActiveSheet()->SetCellValue('O'. $rowCount, $element->amphur_name);
            $objPHPExcel->getActiveSheet()->SetCellValue('P'. $rowCount, $element->province_name);
            $objPHPExcel->getActiveSheet()->SetCellValue('Q'. $rowCount, $element->zipcode);
            $objPHPExcel->getActiveSheet()->SetCellValue('R'. $rowCount, $element->category_name);
            $objPHPExcel->getActiveSheet()->SetCellValue('S'. $rowCount, $element->position_name);
            $objPHPExcel->getActiveSheet()->SetCellValue('T'. $rowCount, $element->level_name);
            $objPHPExcel->getActiveSheet()->SetCellValue('U'. $rowCount, $element->department_name);
            $objPHPExcel->getActiveSheet()->SetCellValue('V'. $rowCount, $element->section_name);
            $rowCount++;
            $no++;
        }
